...
Eigen::VectorXd linear_single_solve_test(int linear_solver, const Eigen::VectorXd y, 
    const Eigen::ArrayXd weights, Eigen::VectorXd x, double rho, 
    const Eigen::VectorXd adj_mean) {
  int n = y.size();
  int k = n - adj_mean.size();
  SparseMatrix<double> dk_mat = get_dk_mat(k, Rcpp::wrap(x), false);
  ...
  Eigen::MatrixXd denseD;
  Eigen::VectorXd s_seq;
  if (linear_solver == 2) {
    ...
    configure_denseD(x, denseD, s_seq, k, equal_space);
  }
  ...
  std::tie(sol, info) = linear_system.solve(y, weights, adj_mean, k, Rcpp::wrap(x), rho, denseD, 
